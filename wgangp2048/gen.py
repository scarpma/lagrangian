...
def build_generator(fs, fm, init_sigma, init_mean, alpha, noise_dim):
    ...
    # Starting size
    generator.add(Dense(64*fm, kernel_regularizer=reg, bias_regularizer=reg, kernel_initializer=RandomNormal(init_mean, init_sigma), input_dim=noise_dim))
    #generator.add(ELU())
    generator.add(ReLU())
    #20x1
    ...
    generator.add(Conv2DTranspose(fm//2, fs, strides=(2,1), padding='same', kernel_regularizer=reg, bias_regularizer=reg, kernel_initializer=RandomNormal(init_mean, init_sigma)))
    #generator.add(ELU())
    generator.add(ReLU())
    #50x4
    generator.add(BatchNormalization(momentum=0.8))
