...
class WGANGP():
    ...
    def plot_trajs(self, gen_trajs,epoch):
        plt.figure(figsize=(15, 3*2*gen_trajs.shape[0]))
        for i, traj in enumerate(gen_trajs):
            plt.subplot(3*gen_trajs.shape[0], 1, 3*i+1)
            plt.plot(traj[:,0],label=str(i)+" x")
            plt.subplot(3*gen_trajs.shape[0], 1, 3*i+2)
            plt.plot(traj[:,1],label=str(i)+" y")
            plt.subplot(3*gen_trajs.shape[0], 1, 3*i+3)
            plt.plot(traj[:,2],label=str(i)+" z")
        plt.set_legend()
        plt.tight_layout()
