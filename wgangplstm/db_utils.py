...
from tensorflow.keras.layers import Dense, Conv1D, Conv2D, Conv2DTranspose, Flatten, Dropout, ReLU, ELU, Input, MaxPooling1D, Reshape, BatchNormalization, Activation, Bidirectional, LSTM
...
def load_data(val_split):
    path = '/storage/scarpolini/databases/velocities.npy'
    db = np.load(path)[:,:,0:1]
    print(db.shape)
    M = db.max()
    m = db.min()
